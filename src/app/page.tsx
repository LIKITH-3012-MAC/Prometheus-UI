    ...
    <main className="fixed inset-0 bg-[#020617] flex flex-col font-sans overflow-hidden">
      ...
      <header className="w-full h-20 flex justify-between items-center px-6 md:px-12 z-50 bg-[#020617]/80 backdrop-blur-xl border-b border-white/5">
        <div className="flex flex-col">
          ...
          <p className="text-[9px] tracking-[0.4em] font-bold text-cyan-400 opacity-60 uppercase">Neural Link • Likith Naidu</p>
        </div>
      ...
      {/* 2. Isolated Chat Area */}
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 md:px-[25%] py-10 space-y-10 scroll-smooth custom-scrollbar">
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center space-y-4 opacity-20">
            ...
            <p className="text-[10px] tracking-[2em] font-black text-white uppercase text-center">System Ready</p>
          </div>
        )}
        ...
        {loading && <div className="text-cyan-500 animate-pulse text-[10px] font-black uppercase tracking-widest px-4 italic">Processing Command...</div>}
      </div>

      {/* 3. 🔥 FIXED SEARCH BAR WITH VISIBLE TEXT */}
      <footer className="w-full h-32 flex items-center justify-center px-4 md:px-12 z-50 bg-gradient-to-t from-[#020617] via-[#020617]/90 to-transparent">
        <div className="w-full max-w-3xl glass-strong flex items-center p-2 rounded-[32px] border-white/10 shadow-[0_-20px_50px_rgba(0,0,0,0.8)] focus-within:border-cyan-500/50 transition-all duration-500">
          <input 
            value={input} 
            onKeyDown={(e) => e.key === 'Enter' && handleSend()} 
            onChange={(e) => setInput(e.target.value)} 
            className="flex-1 bg-transparent px-8 py-5 outline-none text-white text-lg placeholder:text-zinc-600 font-medium" 
